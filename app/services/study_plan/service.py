    ...
    *,
    ...
    progress = (
        build_weekly_progress(
            ...
            completed_minutes_this_week=int(weekly_metrics['completed_minutes_this_week']),
            answered_questions_this_week=int(weekly_metrics['answered_questions_this_week']),
        )
        if configured
        else {
...
def fetch_study_plan(db, *, user_id: int) -> dict[str, object]:
    ...
    return _build_payload(
        configured=True,
        study_days_per_week=int(row.get('study_days_per_week') or DEFAULT_STUDY_DAYS_PER_WEEK),
        ...
        preferred_period=str(row.get('preferred_period') or DEFAULT_PREFERRED_PERIOD),
...
def save_study_plan(
    ...
    *,
    ...
    preferred_period = _normalize_preferred_period(payload.get('preferred_period'))
